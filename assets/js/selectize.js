...
!function(root, factory) {
    ...
    Sifter.prototype.tokenize = function(query) {
        ...
        for (var regex, letter, tokens = [], words = query.split(/ +/), i = 0, n = words.length; i < n; i++) {
            if (regex = escape_regex(words[i]),
            this.settings.diacritics)
                for (letter in DIACRITICS)
                    DIACRITICS.hasOwnProperty(letter) && (regex = regex.replace(new RegExp(letter,"g"), DIACRITICS[letter]));
            tokens.push({
                string: words[i],
    ...
    Sifter.prototype.getScoreFunction = function(search, options) {
        function scoreValue(score, token) {
            var pos;
            return !score || -1 === (pos = (score = String(score || "")).search(token.regex)) ? 0 : (score = token.string.length / score.length,
            0 === pos && (score += .5),
            score)
        }
        ...
        : "and" === search.options.conjunction ? function(data) {
            for (var score, i = 0, sum = 0; i < token_count; i++) {
                if ((score = scoreObject(tokens[i], data)) <= 0)
                    return 0;
                sum += score
            }
    ...
    Sifter.prototype.getSortFunction = function(search, options) {
        ...
        function(a, b) {
            ...
            for (var result, i = 0; i < fields_count; i++)
                ...
                result = multipliers[i] * cmp(get_field(result, a), get_field(result, b)))
                    return result;
            return 0
        }
        : null
    ...
    Sifter.prototype.prepareSearch = function(query, options) {
        ...
        return option_fields && !is_array(option_fields) && (options.fields = [option_fields]),
    ...
    Sifter.prototype.search = function(fn_sort, options) {
        ...
        return options = search.options,
        fn_sort = search.query,
        fn_score = options.score || this.getScoreFunction(search),
        fn_sort.length ? this.iterator(this.items, function(item, id) {
            score = fn_score(item),
            (!1 === options.filter || 0 < score) && search.items.push({
                score: score,
                id: id
            })
      ...
      , extend = function(a, b) {
        for (var k, object, i = 1, n = arguments.length; i < n; i++)
            if (object = arguments[i])
                for (k in object)
                    object.hasOwnProperty(k) && (a[k] = object[k]);
        return a
    }
      ...
      , asciifold = function() {
        ...
        for (k in DIACRITICS)
            if (DIACRITICS.hasOwnProperty(k))
                for (foreignletters += chunk = DIACRITICS[k].substring(2, DIACRITICS[k].length - 1),
                i = 0,
                n = chunk.length; i < n; i++)
        ...
        mixin: function(Interface) {
            ...
            Interface.prototype.initializePlugins = function(plugins) {
                ...
                utils.isArray(plugins))
                    for (i = 0,
                    n = plugins.length; i < n; i++)
                        "string" == typeof plugins[i] ? queue.push(plugins[i]) : (this.plugins.settings[plugins[i].name] = plugins[i].options,
                        queue.push(plugins[i].name));
                else if (plugins)
                    for (key in plugins)
                        plugins.hasOwnProperty(key) && (this.plugins.settings[key] = plugins[key],
                        queue.push(key));
                for (; queue.length; )
                    this.require(queue.shift())
            }
            ,
...
function(root, factory) {
    ...
    function getSelection(input) {
        ...
        return void 0 === input ? console.warn("WARN getSelection cannot locate input control") : "selectionStart"in input ? (result.start = input.selectionStart,
    ...
    function measureString(str, $parent) {
        ...
        function($from, $to, properties) {
            ...
            if (properties)
                ...
                styles = $from.css();
            $to.css(styles)
        }($parent, Selectize.$testInput, ["letterSpacing", "fontSize", "fontFamily", "fontWeight", "textTransform"]),
    ...
    function autoGrow($input) {
        function update(e, selection) {
            ...
            selection = selection || {},
            (e = e || window.event || {}).metaKey || e.altKey || !selection.force && !1 === $input.data("grow") || (width = $input.val(),
            e.type && "keydown" === e.type.toLowerCase() && (shift = 48 <= (keyCode = e.keyCode) && keyCode <= 57 || 65 <= keyCode && keyCode <= 90 || 96 <= keyCode && keyCode <= 111 || 186 <= keyCode && keyCode <= 222 || 32 === keyCode,
            46 === keyCode || 8 === keyCode ? (selection = getSelection($input[0])).length ? width = width.substring(0, selection.start) + width.substring(selection.start + selection.length) : 8 === keyCode && selection.start ? width = width.substring(0, selection.start - 1) + width.substring(selection.start + 1) : 46 === keyCode && void 0 !== selection.start && (width = width.substring(0, selection.start) + width.substring(selection.start + 1)) : shift && (shift = e.shiftKey,
            placeholder = String.fromCharCode(e.keyCode),
            width += placeholder = shift ? placeholder.toUpperCase() : placeholder.toLowerCase())),
            placeholder = $input.attr("placeholder"),
            (width = measureString(width = !width && placeholder ? placeholder : width, $input) + 4) !== currentWidth && (currentWidth = width,
            $input.width(width),
    ...
    var IS_MAC = /Mac/.test(navigator.userAgent)
      ...
      , Selectize = function($input, settings) {
        ...
        if (dir = (dir = dir ? dir.getPropertyValue("direction") : input.currentStyle && input.currentStyle.direction) || $input.parents("[dir]:first").attr("dir") || "",
        $.extend(this, {
            order: 0,
            settings: settings,
            $input: $input,
        ...
        this.settings.options) {
            for (i = 0,
            n = this.settings.options.length; i < n; i++)
                this.registerOption(this.settings.options[i]);
            delete this.settings.options
        }
        if (this.settings.optgroups) {
            for (i = 0,
            n = this.settings.optgroups.length; i < n; i++)
    ...
    return MicroEvent.mixin(Selectize),
    void 0 !== MicroPlugin ? MicroPlugin.mixin(Selectize) : function(message, options) {
        options = options || {};
        console.error("Selectize: " + message),
        options.explanation && (console.group && console.group(),
    ...
    $.extend(Selectize.prototype, {
        setup: function() {
            ...
            self.plugins.names.length && (delimiterEscaped = "plugin-" + self.plugins.names.join(" plugin-"),
            ...
            !self.settings.splitOn && self.settings.delimiter && (delimiterEscaped = self.settings.delimiter.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&"),
            ...
            selector.on("mouseenter mousedown click", "[data-disabled]>[data-selectable]", function(e) {
            ...
            ($parent = $control).on(event, selector, function(e) {
                for (var child = e.target; child && child.parentNode !== $parent[0]; )
                    child = child.parentNode;
                return e.currentTarget = child,
                fn.apply(this, [e])
            }),
            ...
            Array.isArray(settings.items) && (self.lastValidValue = settings.items,
            self.setValue(settings.items),
            delete settings.items),
            SUPPORTS_VALIDITY_API && $input.on("invalid" + eventNS, function(e) {
                e.preventDefault(),
                self.isInvalid = !0,
                self.refreshState()
            }),
        ...
        onMouseDown: function(e) {
            ...
            if (self.isFocused) {
                if (e.target !== self.$control_input[0])
                    return "single" === self.settings.mode ? self.isOpen ? self.close() : self.open() : defaultPrevented || self.setActiveItem(null),
                    !1
            } else
                defaultPrevented || window.setTimeout(function() {
                    self.focus()
                }, 0)
        },
        onChange: function() {
        ...
        onKeyPress: function(e) {
            ...
            return this.settings.create && "multi" === this.settings.mode && character === this.settings.delimiter ? (this.createItem(),
            e.preventDefault(),
            !1) : void 0
        },
        onKeyDown: function(e) {
            ...
            if (this.isLocked)
                ...
                case 65:
                    ...
                    break;
                case 27:
                    return void (this.isOpen && (e.preventDefault(),
                    e.stopPropagation(),
                    this.close()));
                case 78:
                ...
                case 40:
                    ...
                    ($prev = this.getAdjacentOption(this.$activeOption, 1)).length && this.setActiveOption($prev, !0, !0)),
                    void e.preventDefault();
                case 80:
                    if (!e.ctrlKey || e.altKey)
                        break;
                case 38:
                    return this.$activeOption && (this.ignoreHover = !0,
                    ($prev = this.getAdjacentOption(this.$activeOption, -1)).length && this.setActiveOption($prev, !0, !0)),
                    void e.preventDefault();
                case 13:
        ...
        onKeyUp: function(value) {
            if (this.isLocked)
                return value && value.preventDefault();
        ...
        onBlur: function(deactivate, dest) {
            var self = this;
            if (self.isFocused && (self.isFocused = !1,
            !self.ignoreFocus)) {
                if (!self.ignoreBlur && document.activeElement === self.$dropdown_content[0])
                    return self.ignoreBlur = !0,
                    void self.onFocus(deactivate);
                deactivate = function() {
                    self.close(),
                ...
                self.isBlurring = !0,
                self.ignoreFocus = !0,
                self.settings.create && self.settings.createOnBlur ? self.createItem(null, !1, deactivate) : deactivate()
            }
        },
        ...
        onOptionSelect: function(e) {
            ...
            e.stopPropagation()),
            (value = $(e.currentTarget)).hasClass("create") ? self.createItem(null, function() {
                self.settings.closeAfterSelect && self.close()
            }) : void 0 !== (value = value.attr("data-value")) && (self.lastQuery = null,
            self.setTextboxValue(""),
            self.addItem(value),
            self.settings.closeAfterSelect ? self.close() : !self.settings.hideSelected && e.type && /mouse/.test(e.type) && self.setActiveOption(self.getOption(value)))
        },
        ...
        load: function(fn) {
            ...
            fn.apply(self, [function(results) {
                ...
                results && results.length && (self.addOption(results),
        ...
        setActiveItem: function($item, e) {
            ...
            if ("single" !== this.settings.mode) {
                if (!($item = $($item)).length)
                    return $(this.$activeItems).removeClass("active"),
                    this.$activeItems = [],
                    void (this.isFocused && this.showInput());
                if ("mousedown" === (idx = e && e.type.toLowerCase()) && this.isShiftDown && this.$activeItems.length) {
                    for (swap = this.$control.children(".active:last"),
                    begin = Array.prototype.indexOf.apply(this.$control[0].childNodes, [swap[0]]),
                    (end = Array.prototype.indexOf.apply(this.$control[0].childNodes, [$item[0]])) < begin && (swap = begin,
                    begin = end,
                    end = swap),
                    i = begin; i <= end; i++)
                    ...
                    "mousedown" === idx && this.isCtrlDown || "keydown" === idx && this.isShiftDown ? $item.hasClass("active") ? (idx = this.$activeItems.indexOf($item[0]),
                    this.$activeItems.splice(idx, 1),
                    $item.removeClass("active")) : this.$activeItems.push($item.addClass("active")[0]) : ($(this.$activeItems).removeClass("active"),
        ...
        setActiveOption: function(scroll_bottom, scroll, animate) {
            ...
            this.trigger("dropdown_item_deactivate", this.$activeOption.attr("data-value"))),
            this.$activeOption = null,
            (scroll_bottom = $(scroll_bottom)).length && (this.$activeOption = scroll_bottom.addClass("active"),
            this.isOpen && this.trigger("dropdown_item_activate", this.$activeOption.attr("data-value")),
            !scroll && isset(scroll) || (height_menu = this.$dropdown_content.height(),
            height_item = this.$activeOption.outerHeight(!0),
            scroll = this.$dropdown_content.scrollTop() || 0,
            scroll_bottom = (scroll_top = y = this.$activeOption.offset().top - this.$dropdown_content.offset().top + scroll) - height_menu + height_item,
            height_menu + scroll < y + height_item ? this.$dropdown_content.stop().animate({
        ...
        focus: function() {
            var self = this;
            return self.isDisabled || (self.ignoreFocus = !0,
        ...
        blur: function(dest) {
            return this.$control_input[0].blur(),
        ...
        search: function(query) {
            ...
            if (query !== this.lastQuery ? (this.lastQuery = query,
            result = this.sifter.search(query, $.extend(options, {
        ...
        refreshOptions: function(triggerDropdown) {
            ...
            var d, tmp, self = this, query = self.$control_input.val().trim(), results = self.search(query), $dropdown_content = self.$dropdown_content, $active_before = self.$activeOption && hash_key(self.$activeOption.attr("data-value")), n = results.items.length;
            for ("number" == typeof self.settings.maxOptions && (n = Math.min(n, self.settings.maxOptions)),
            groups = {},
            groups_order = [],
            i = 0; i < n; i++)
                for (option = self.options[results.items[i].id],
                option_html = self.render("option", option),
                optgroup = option[self.settings.optgroupField] || "",
                j = 0,
                k = (optgroups = Array.isArray(optgroup) ? optgroup : [optgroup]) && optgroups.length; j < k; j++)
                    optgroup = optgroups[j],
                    self.optgroups.hasOwnProperty(optgroup) || (optgroup = ""),
                    groups.hasOwnProperty(optgroup) || (groups[optgroup] = document.createDocumentFragment(),
            ...
            i = 0,
            n = groups_order.length; i < n; i++)
                optgroup = groups_order[i],
                self.optgroups.hasOwnProperty(optgroup) && groups[optgroup].childNodes.length ? ((html_children = document.createDocumentFragment()).appendChild(self.render("optgroup_header", self.optgroups[optgroup])),
                html_children.appendChild(groups[optgroup]),
                html.appendChild(self.render("optgroup", $.extend({}, self.optgroups[optgroup], {
                    html: (d = html_children,
                    tmp = void 0,
                    (tmp = document.createElement("div")).appendChild(d.cloneNode(!0)),
                    tmp.innerHTML),
                    dom: html_children
                })))) : html.appendChild(groups[optgroup]);
            if ($dropdown_content.html(html),
            self.settings.highlight && ($dropdown_content.removeHighlight(),
            results.query.length && results.tokens.length))
                for (i = 0,
                n = results.tokens.length; i < n; i++)
                            ...
                            var regex = "string" == typeof pattern ? new RegExp(pattern,"i") : pattern
                              , highlight = function(node) {
                                ...
                                if (3 === node.nodeType) {
                                    ...
                                    0 <= middlebit && 0 < node.data.length && (middleclone = node.data.match(regex),
            ...
            if (!self.settings.hideSelected)
                for (self.$dropdown.find(".selected").removeClass("selected"),
                i = 0,
                n = self.items.length; i < n; i++)
            ...
            self.hasOptions ? (0 < results.items.length ? ($active_before = $active_before && self.getOption($active_before),
            "" !== results.query && $active_before && $active_before.length ? $active = $active_before : "single" === self.settings.mode && self.items.length && ($active = self.getOption(self.items[0])),
            ...
            triggerDropdown && !self.isOpen && self.open()) : (self.setActiveOption(null),
        ...
        addOption: function(data) {
            ...
            if (Array.isArray(data))
                ...
                (value = this.registerOption(data)) && (this.userOptions[value] = !0,
                this.lastQuery = null,
        ...
        registerOptionGroup: function(data) {
            ...
            return !!key && (data.$order = data.$order || ++this.order,
        ...
        removeOptionGroup: function(id) {
            this.optgroups.hasOwnProperty(id) && (delete this.optgroups[id],
            this.renderCache = {},
        ...
        updateOption: function($item, $item_new) {
            ...
            null !== $item && this.options.hasOwnProperty($item)) {
                ...
                value_new !== $item && (delete this.options[$item],
                -1 !== (cache_items = this.items.indexOf($item)) && this.items.splice(cache_items, 1, value_new)),
                ...
                cache_items && (delete cache_items[$item],
                delete cache_items[value_new]),
                cache_options && (delete cache_options[$item],
                delete cache_options[value_new]),
                -1 !== this.items.indexOf(value_new) && ($item = this.getItem($item),
        ...
        getElementWithTextContent: function(textContent, ignoreCase, $els) {
            if (null != (textContent = hash_key(textContent)))
                for (var i = 0, n = $els.length; i < n; i++) {
                    ...
                    if (1 == ignoreCase && (eleTextContent = null !== eleTextContent ? eleTextContent.toLowerCase() : null,
        ...
        addItems: function(control, silent) {
            ...
            for (var childNodes = this.$control[0].childNodes, i = 0; i < childNodes.length; i++)
                this.buffer.appendChild(childNodes[i]);
            for (var items = Array.isArray(control) ? control : [control], i = 0, n = items.length; i < n; i++)
        ...
        addItem: function(value, silent) {
            debounce_events(this, silent ? [] : ["change"], function() {
                ...
                value = hash_key(value),
                -1 === this.items.indexOf(value) ? this.options.hasOwnProperty(value) && ("single" === inputMode && this.clear(silent),
                "multi" === inputMode && this.isFull() || ($item = $(this.render("item", this.options[value])),
                value_next = this.isFull(),
                this.items.splice(this.caretPos, 0, value),
                this.insertAtCaret($item),
                this.isPending && (value_next || !this.isFull()) || this.refreshState(),
                this.isSetup && ($options = this.$dropdown_content.find("[data-selectable]"),
                this.isPending || (value_next = this.getOption(value),
                value_next = this.getAdjacentOption(value_next, 1).attr("data-value"),
                this.refreshOptions(this.isFocused && "single" !== inputMode),
                value_next && this.setActiveOption(this.getOption(value_next))),
                !$options.length || this.isFull() ? this.close() : this.isPending || this.positionDropdown(),
        ...
        removeItem: function(value, silent) {
            ...
            -1 !== (i = this.items.indexOf(value)) && (this.trigger("item_before_remove", value, $item),
            $item.remove(),
        ...
        createItem: function(output, triggerDropdown) {
            ...
            var fn, called, setup = "function" == typeof self.settings.create ? this.settings.create : function(key) {
                ...
                if (self.settings.formatValueToKey && "function" == typeof self.settings.formatValueToKey && (null == (key = self.settings.formatValueToKey.apply(this, [key])) || "object" == typeof key || "function" == typeof key))
                    throw new Error('Selectize "formatValueToKey" setting must be a function that returns a value other than object or function.');
                return data[self.settings.valueField] = key,
                data
            }
            ...
            return void 0 !== output && create(output),
            !0
        },
        ...
        refreshClasses: function() {
            ...
            this.$control.toggleClass("focus", this.isFocused).toggleClass("disabled", this.isDisabled).toggleClass("required", this.isRequired).toggleClass("invalid", this.isInvalid).toggleClass("locked", isLocked).toggleClass("full", isFull).toggleClass("not-full", !isFull).toggleClass("input-active", this.isFocused && !this.isInputHidden).toggleClass("dropdown-active", this.isOpen).toggleClass("has-options", !$.isEmptyObject(this.options)).toggleClass("has-items", 0 < this.items.length),
        ...
        updatePlaceholder: function() {
            var $input;
            this.settings.placeholder && ($input = this.$control_input,
        ...
        insertAtCaret: function(target) {
            ...
            0 === caret ? target.insertBefore(el, target.firstChild) : target.insertBefore(el, target.childNodes[caret]),
            this.setCaret(caret + 1)
        },
        deleteSelection: function(e) {
            var i, n, values, option_select, $option_select, caret, direction = e && 8 === e.keyCode ? -1 : 1, selection = getSelection(this.$control_input[0]);
            if (this.$activeOption && !this.settings.hideSelected && (option_select = ("string" == typeof this.settings.deselectBehavior && "top" === this.settings.deselectBehavior ? this.getFirstOption() : this.getAdjacentOption(this.$activeOption, -1)).attr("data-value")),
            values = [],
            this.$activeItems.length) {
                for (caret = this.$control.children(".active:" + (0 < direction ? "last" : "first")),
                ...
                (this.isFocused || "single" === this.settings.mode) && this.items.length && (direction < 0 && 0 === selection.start && 0 === selection.length ? values.push(this.items[this.caretPos - 1]) : 0 < direction && selection.start === this.$control_input.val().length && values.push(this.items[this.caretPos]));
            if (!values.length || "function" == typeof this.settings.onDelete && !1 === this.settings.onDelete.apply(this, [values]))
                return !1;
            ...
            option_select && ($option_select = this.getOption(option_select)).length && this.setActiveOption($option_select),
            !0
        },
        advanceSelection: function(direction, e) {
            var selection, valueLength, idx;
            0 !== direction && (this.rtl && (direction *= -1),
            idx = 0 < direction ? "last" : "first",
            selection = getSelection(this.$control_input[0]),
            this.isFocused && !this.isInputHidden ? (valueLength = this.$control_input.val().length,
            (direction < 0 ? 0 !== selection.start || 0 !== selection.length : selection.start !== valueLength) || valueLength || this.advanceCaret(direction, e)) : (idx = this.$control.children(".active:" + idx)).length && (idx = this.$control.children(":not(input)").index(idx),
            this.setActiveItem(null),
        ...
        advanceCaret: function(direction, e) {
            ...
            e && e.preventDefault()) : this.setCaret(this.caretPos + direction))
        },
        setCaret: function(i) {
        ...
        render: function(templateName, data) {
            ...
            return (cache = "option" === templateName || "item" === templateName ? !!(value = hash_key(data[this.settings.valueField])) : cache) && (isset(this.renderCache[templateName]) || (this.renderCache[templateName] = {}),
            this.renderCache[templateName].hasOwnProperty(value)) ? this.renderCache[templateName][value] : (html = $(this.settings.render[templateName].apply(this, [data, escape_html])),
            "option" === templateName || "option_create" === templateName ? data[this.settings.disabledField] || html.attr("data-selectable", "") : "optgroup" === templateName && (id = data[this.settings.optgroupValueField] || "",
            html.attr("data-group", id),
            data[this.settings.disabledField] && html.attr("data-disabled", "")),
    ...
    $.fn.selectize = function(settings_user) {
        function init_select($input, settings_element) {
            ...
            function addOption($option, group) {
                ...
                (value || settings.allowEmptyOption) && (optionsMap.hasOwnProperty(value) ? group && ((option = optionsMap[value][field_optgroup]) ? $.isArray(option) ? option.push(group) : optionsMap[value][field_optgroup] = [option, group] : optionsMap[value][field_optgroup] = group) : ((option = readData($option) || {})[field_label] = option[field_label] || $option.text(),
            ...
            i = 0,
            n = ($children = $input.children()).length; i < n; i++)
                "optgroup" === (tagName = $children[i].tagName.toLowerCase()) ? function($optgroup) {
                    ...
                    i = 0,
                    n = ($options = $("option", $optgroup)).length; i < n; i++)
        ...
        return this.each(function() {
            ...
            this.selectize || ($input = $(this),
            tag_name = this.tagName.toLowerCase(),
            (settings_element = $input.attr("placeholder") || $input.attr("data-placeholder")) || settings.allowEmptyOption || (settings_element = $input.children('option[value=""]').text()),
            settings.allowEmptyOption && settings.showEmptyOptionInDropdown && !$input.children('option[value=""]').length && (input_html = $input.html(),
            ...
            ("select" === tag_name ? init_select : function(value, settings_element) {
                ...
                if (data_raw)
                    for (settings_element.options = JSON.parse(data_raw),
                    i = 0,
                    n = settings_element.options.length; i < n; i++)
                        settings_element.items.push(settings_element.options[i][field_value]);
                else {
                    value = $.trim(value.val() || "");
                    if (settings.allowEmptyOption || value.length) {
                        for (i = 0,
                        n = (values = value.split(settings.delimiter)).length; i < n; i++)
    ...
    Selectize.define("auto_position", function() {
        ...
        this.positionDropdown = function() {
            const $control = this.$control
              , offset = "body" === this.settings.dropdownParent ? $control.offset() : $control.position();
            offset.top += $control.outerHeight(!0);
            var styles = this.$dropdown.prop("scrollHeight") + 5
              , position = this.$control.get(0).getBoundingClientRect().top + styles + this.$wrapper.height() > window.innerHeight ? POSITION.top : POSITION.bottom
              , styles = {
                width: $control.outerWidth(),
                left: offset.left
            };
            position === POSITION.top ? (Object.assign(styles, {
                bottom: offset.top,
                top: "unset",
                margin: "0 0 5px 0"
            }),
    ...
    Selectize.define("auto_select_on_type", function(options) {
        ...
        self.onBlur = (originalBlur = self.onBlur,
        function(e) {
            var $matchedItem = self.getFirstItemMatchedByTextContent(self.lastValue, !0);
            return void 0 !== $matchedItem.attr("data-value") && self.getValue() !== $matchedItem.attr("data-value") && self.setValue($matchedItem.attr("data-value")),
    ...
    Selectize.define("drag_drop", function(options) {
        ...
        function() {
            ...
            return sortable && sortable.disable(),
            original.apply(self, arguments)
        }
        ),
        self.unlock = function() {
            ...
            return function() {
                var sortable = self.$control.data("sortable");
                return sortable && sortable.enable(),
                original.apply(self, arguments)
            }
        }(),
        self.setup = function() {
    ...
    Selectize.define("optgroup_columns", function(options) {
        ...
        function(e) {
            var $option, $options;
            return !this.isOpen || 37 !== e.keyCode && 39 !== e.keyCode ? original.apply(this, arguments) : (self.ignoreHover = !0,
            $option = ($options = this.$activeOption.closest("[data-group]")).find("[data-selectable]").index(this.$activeOption),
            void (($option = ($options = ($options = 37 === e.keyCode ? $options.prev("[data-group]") : $options.next("[data-group]")).find("[data-selectable]")).eq(Math.min($options.length - 1, $option))).length && this.setActiveOption($option)))
        }
        );
        function equalizeSizes() {
            var i, height_max, width_last, width_parent, $optgroups = $("[data-group]", self.$dropdown_content), n = $optgroups.length;
            if (n && self.$dropdown_content.width()) {
                ...
                1 < n && (width_last = width_parent - width_last * (n - 1),
    ...
    Selectize.define("remove_button", function(options) {
        ...
        ("single" === this.settings.mode ? function(thisRef, options) {
            ...
            thisRef.setup = (original = self.setup,
            function() {
                var id, render_item;
                options.append && (id = $(self.$input.context).attr("id"),
                $("#" + id),
        ...
        : function(thisRef, options) {
            ...
            function() {
                var render_item;
                options.append && (render_item = self.settings.render.item,
                self.settings.render.item = function(data) {
                    return html_container = render_item.apply(thisRef, arguments),
                    html_element = html,
                ...
                original.apply(thisRef, arguments),
                thisRef.$control.on("click", "." + options.className, function($item) {
                    if ($item.preventDefault(),
                    !self.isLocked) {
                        $item = $($item.currentTarget).parent();
                        return self.setActiveItem($item),
                        self.deleteSelection() && self.setCaret(self.items.length),
                        !1
                    }
                })
            }
    ...
    Selectize.define("restore_on_backspace", function(options) {
        ...
        function(e) {
            var option;
            return 8 === e.keyCode && "" === this.$control_input.val() && !this.$activeItems.length && 0 <= (option = this.caretPos - 1) && option < this.items.length ? (option = this.options[this.items[option]],
            this.deleteSelection(e) && (this.setTextboxValue(options.text.apply(this, [option])),
            this.refreshOptions(!0)),
            void e.preventDefault()) : original.apply(this, arguments)
        }
        )
    }),
    Selectize.define("select_on_focus", function(options) {
        var originalFocus, originalBlur, self = this;
        self.on("focus", (originalFocus = self.onFocus,
        function(e) {
            ...
            setTimeout(function() {
                self.settings.selectOnTab && self.setActiveOption(self.getFirstItemMatchedByTextContent(value)),
        ...
        self.onBlur = (originalBlur = self.onBlur,
        function(e) {
            return "" === self.getValue() && self.lastValidValue !== self.getValue() && self.setValue(self.lastValidValue),
            setTimeout(function() {
    ...
    Selectize.define("tag_limit", function(options) {
        ...
        this.onBlur = function() {
            ...
            return function(e) {
                if (original.apply(this, e),
                e) {
                    const $control = this.$control
                      , $items = $control.find(".item")
                      , limit = options.tagLimit;
                    void 0 === limit || $items.length <= limit || ($items.toArray().forEach((item,index)=>{
